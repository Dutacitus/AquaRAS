...
RAS.engine = (function () {
  ...
  function compute(inputs) {
    ...
    const checks = [
      ...
      { key: "salr", name: "生物滤池面积负荷 SALR", value: round(salr, 2), unit: "g TAN/m²·天", limit: `≤${salrMax}（经济 ${salrRef}）`, status: salrStatus, note: `填料体积 ${round(mediaVol, 1)} m³（载体比表面积属 AquaRAS 商业机密）；TAN 日负荷 ${round(tanDaily * 1000, 0)} g/天（对照 Frontiers 2023 MBBR 经验）` },
      ...
      { key: "nh3", name: "非离子氨 NH₃", value: round(cNH3, 4), unit: "mg/L(N)", limit: round(nh3AcuteT, 4), status: cNH3 > nh3AcuteT ? "fail" : (cNH3 > nh3ChronicT ? "warn" : "ok"), note: `TAN ${round(cTan, 2)} × 离解率 ${round(fNH3 * 100, 1)}% (pKa ${round(pKaNH3, 2)})；温度修正阈值属 AquaRAS 商业机密（当前 ${Math.round(temp)}℃）` },

...
RAS.pid = (function () {
  ...
  function render(d) {
    ...
    return `<svg viewBox="0 0 ${W} ${H}" class="pid-svg" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="P&ID 带仪表控制点">
      ...
      <!-- 设备 -->
      ${equip(xs[0], eqY, eqW, eqH, "TK-01", "养殖池组", `${d.culture.tankCount}×Ø${d.culture.tankD}m`, "c1", "tk")}
      ${equip(xs[1], eqY, eqW, eqH, "DR-01", "转鼓微滤机", `${d.solids.screen}µm`, "c2", "dr")}
      ${equip(xs[2], eqY, eqW, eqH, "BF-01", "MBBR 滤池", `${d.biofilter.totalVol}m³`, "c3", "bf")}
      ${equip(xs[3], eqY, eqW, eqH, "OT-01", "增氧+CO₂脱除", `${d.oxygen.o2Supply}kg/h`, "c4", "ot")}
      ${equip(xs[4], eqY, eqW, eqH, "UV-01", "紫外消毒", "30 mJ/cm²", "c5", "uv")}
      ...
      <!-- 换热器(回水管上) -->
      ${equip(540, yRet - 26, 120, 52, "HE-01", "换热器", `${d.inputs.temp}℃`, "c8", "he")}

      <!-- 新鲜补水 + 控制阀 FV-01 -->
      ${equip(xs[4], 420, eqW, eqH, "MAKEUP", "新鲜补水", `${d.hydraulics.makeupFlowH} m³/h`, "c6", "makeup")}
